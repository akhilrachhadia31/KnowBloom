...
const getLeafSize = (icon) => {
  ...
  return "w-9 h-9"; // fixed typo: "w-9 w-9"
};
  ...
  const downloadImage = async () => {
    try {
      const canvas = await html2canvas(logoRef.current, {
        ...
        useCORS: true, // for icon rendering
      });
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        ...
        } flex items-center ${currentSize.spacing} ${className}`}
        style={{ backgroundColor: "white" }} // Avoid Tailwind `oklch`
      >
        <div className="relative">
          <BookOpen className={`${currentSize.icon} text-blue-600`} />
          <Leaf
            className={`${getLeafSize(
              currentSize.icon
            )} text-green-500 absolute -top-1 -right-1 rotate-12`}
          />
        </div>
        {showText && (
          <div
